...
SHELL_FILE=$0
SCF_PATH="/mnt/services/mzoam/config/4.1144.36-R4-B4-NSA"
...
function insert_du_fronthaul_to_scf_oam()
{
    ...
    do
        if [[ "$line" =~ "neType\">FSM" ]]; then
            IFS=$BACK_IFS
            $ECHO_INFO "Change configuration of DU-$index"
            IFS=""
            let du_find_flag=1
        fi
        if [[ $du_find_flag == 1 && "$line" =~ "localIpAddr" ]]; then
            old_ip=`echo "$line" | sed -nr "s/.*localIpAddr\">(.*)<.*/\1/p"`
            fronthaul_ip=`ls .dus/.create_scf/ | grep du$index- | awk -F '-' '{print $2}'`
            IFS=$BACK_IFS
            $ECHO_INFO "OLD $old_ip -> NEW $fronthaul_ip"
            IFS=""
